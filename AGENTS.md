...
- Provider features may use `Prepare(context.Context) error` for fail-fast initialization before controller Kubernetes configuration/manager creation. Standalone subcommands bypass it; existing `Initialize` timing is unchanged.
